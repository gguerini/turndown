...
describe Turndown::Internationalization do
  ...
  let(:subject) {  Turndown::Internationalization }
  ...
  describe '#initialize_i18n' do
    ...
    it 'initializes the i18n ' do
      Turndown.config.i18n.enabled = true
  ...
  describe '#i18n_config' do
    it 'sets the i18n config variable' do
      subject.i18n_config
      expect(subject.instance_variable_get('@i18n_config')).to eq(Turndown.config.i18n)
    end

    it 'sets the i18n config variable from a Hash' do
      original = Turndown.config.i18n
      Turndown.config.i18n = {key: :value }
      expect(Turndown::InheritableOptions).to receive(:new).with(Turndown.config.i18n)
      subject.i18n_config
      Turndown.config.i18n = original
    end
  end

  describe '#turndown_page' do
    it 'sets the turndown page' do
      subject.turndown_page
      expect(subject.instance_variable_get('@turndown_page')).to eq(Turndown.config.default_maintenance_page)
    end
  end
  ...
  describe '#http_accept_language' do
    it 'sets the http_accept_language' do
      subject.env = nil
      expect(Turndown::AcceptLanguageParser).to receive(:new).with(nil)
      subject.http_accept_language
    end

    it 'sets the http_accept_language and instantiates the parser' do
      subject.env = env
      expect(Turndown::AcceptLanguageParser).to receive(:new).with(env['HTTP_ACCEPT_LANGUAGE'])
      subject.http_accept_language
    end
    ...
    it 'sets the http_accept_language' do
      subject.env = env
      subject.http_accept_language
      expect(subject.instance_variable_get('@http_accept_language').kind_of?(Turndown::AcceptLanguageParser)).to eq(true)
    end
  ...
  describe '#setup_additional_helpers' do
    ...
    it 'includes additional helpers' do
      original = Turndown.config.i18n
      Turndown.config.i18n.additional_helpers = helpers
      helpers.each do |helper|
        expected =  helper.is_a?(Module) ? 'to' : 'to_not'
        expect(subject.turndown_page).send(expected, receive(:send).with(:include, helper))
      end
      subject.setup_additional_helpers
      Turndown.config.i18n = original
    end
  ...
  describe '#setup_i18n_config' do
    ...
    context '#railties_load_path' do
      ...
      it 'makes sure that unshift and flatten are happening' do
        original = Turndown.config.i18n
        Turndown.config.i18n.railties_load_path = [locales_dir]
      ...
      it 'sets the load path from single paths' do
        I18n.load_path = []
        Turndown.config.i18n.railties_load_path = [de_locale, en_locale]
        subject.setup_i18n_config
        expect(I18n.load_path).to eq([de_locale, en_locale])
      end
    end
    context '#load_path' do
      it 'add to the load path from array' do
        I18n.load_path = []
        Turndown.config.i18n.railties_load_path = []
        Turndown.config.i18n.load_path = fake_paths
        subject.setup_i18n_config
        expect(I18n.load_path).to eq(fake_paths)
        Turndown.config.i18n.load_path = []
      end
    end
    ...
    context 'send dynamic methods to I18n' do
      it 'add to the load path from array' do
        original = Turndown.config.i18n
        Turndown.config.i18n = { key1: :value1,  key2: :value2, enabled: true }
        Turndown.config.i18n.select{|key, value| ![:enabled].include?(key) }.each do |key, value|
  ...
  describe '#init_fallbacks' do
    ...
    it 'uses ordered options' do
      hash = Turndown::OrderedOptions.new({ map: :value })
