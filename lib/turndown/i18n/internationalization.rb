...
module Turndown
  class Internationalization
    class << self
      ...
      def i18n_config
        @i18n_config = Turndown.config.i18n
        @i18n_config =  @i18n_config.is_a?(Turndown::OrderedOptions) ? @i18n_config : Turndown::InheritableOptions.new(@i18n_config)
      end

      def turndown_page
        @turndown_page ||= Turndown.config.default_maintenance_page
      end

      def http_accept_language
        language = (env.nil? || env.empty?) ? nil : env["HTTP_ACCEPT_LANGUAGE"]
        @http_accept_language ||= Turndown::AcceptLanguageParser.new(language)
      end
      ...
      def setup_additional_helpers
        ...
        i18n_additional_helpers.each do |helper|
          turndown_page.send(:include, helper) if helper.is_a?(Module)
        end
      end
